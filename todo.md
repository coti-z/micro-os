...
### Step 1 — struct file + fd 테이블 ✅
- [x] `kernel/file.h` 작성: `struct file` (inode_no, offset, refcount, type) 정의
- [x] `kernel/file.c` 작성: `file_alloc()` / `file_free()` 구현
- [x] `proc/process.h` PCB에 `struct file *fd_table[16]` 추가
- [x] `proc/process.c` 프로세스 생성 시 fd 테이블 초기화

### Step 2 — stdin / stdout / stderr 기본 연결 ✅
- [x] fd 0 = stdin: type=STDIN, 키보드에서 읽기
- [x] fd 1 = stdout: type=STDOUT, VGA에 쓰기
- [x] fd 2 = stderr: type=STDOUT, VGA에 쓰기
- [x] 모든 프로세스 생성 시 자동으로 fd 0/1/2 세팅

### Step 3 — sys_open / sys_close ✅
- [x] `sys_open(path, flags)`: ext2 경로 탐색 → struct file 생성 → fd 슬롯 반환
- [x] `sys_close(fd)`: refcount 감소, 0이면 file_free
- [x] 유저 프로그램에서 `open("/hello.txt", 0)` 호출 확인 ✓

### Step 4 — sys_read ✅
- [x] `sys_read(fd, buf, n)`: fd 타입에 따라 분기
  - fd=0 (stdin) → keyboard_getchar() 반복 호출 → 유저 버퍼로 복사
  - 파일 fd → ext2_read_file_at(offset 기반) → 유저 버퍼로 복사
- [x] 유저 프로그램에서 파일 읽기 확인 ✓
- [x] `ext2_read_file_at(ino, buf, offset, n)` 구현 (offset 기반 부분 읽기)

### Step 5 — sys_write fd 경유 + sys_lseek ✅
- [x] `sys_write(fd, buf, n)`: fd 타입 기반으로 분기 (fd → file → type 경유)
- [x] `sys_lseek(fd, offset, whence)`: SEEK_SET/SEEK_CUR/SEEK_END 처리
- [x] syscall 번호 Linux ABI 맞춤: read=0, write=1, open=2, close=3, lseek=8, exit=60

### Step 6 — VFS file_ops 추상화 (Phase 14 파이프 대비)
- [ ] `struct file_ops { read, write, seek }` 함수 포인터 테이블
- [ ] ext2 파일, stdin, stdout 각각 file_ops 구현
- [ ] sys_read/write가 fd → file → file_ops 경유로 동작하도록 교체
